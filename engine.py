"""
This program builds an array of numbers from 1 to the user-specified length.
It then allows the user to search for a number in the array and insert it at the correct position.

The program also allows the user to display the array in ascending and descending order.

I learned how to build a Python program that accepts user input and inserts a new number 
into an already sorted list at the correct position without calling the sort() function. 

I explored how adaptive insertion works: by scanning the list for the right index and placing
 the new value directly where it belongs, which is more efficient than sorting the whole array after every change.

I practiced working with core programming concepts like list indexing, loops, user input, and 
conditional logic (if, for). I used list slicing to create a reversed, descending copy of my sorted array.

I improved my debugging and testing skills by writing a trace function to measure how long 
operations took and by setting up test cases to check my logic automatically. I learned how to handle real data, track runtime, and design flexible code that can scale to large arrays.

Finally, I saw the differences between sorting a whole list with .sort() and inserting a 
value adaptively into a sorted structure, and why that matters in real applications for performance and efficiency.
For your adaptive insert algorithm, the steps are as follows:

1. Find the position:
    - Check the array to see if the key is already the highest number
    - if the key is the highest number, return the last index of the array
    - this step takes O(1) time
    - Scan the list from start to finish until you find where the new number goes.
    - This step takes O(n) time—“n” is the number of elements in your list.

2. Inserting the value:
    - Move items over to make space.
    - In the worst case (inserting at the start), this is also O(n) time.

3. Overall:
    - Each insertion is O(n)—linear time.

4. Compare to .sort():
    - If you just append and then call sort(), Python's sort is about O(n log n).
    - Adaptive insertion is faster for inserting one value (O(n) < O(n log n)), especially for large arrays.
"""
import sys, time
import unittest
